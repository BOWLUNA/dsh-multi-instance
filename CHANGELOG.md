...
- **`npm run smoke` —— 窗口级冒烟测试**（`tools/smoke.js`）：起一次真应用，看网页有没有真的被渲染出来。
  ...
  另有 `--exe="dist/win-unpacked/DSH Multi-Instance.exe"`：**直接把打包产物拉起来验一遍** ——
  「源码能跑」和「发出去的那个 exe 能跑」是两件事（后者的 `main.js` 是从 `app.asar` 里读的，
  漏文件只会在这条路径上现形）。0.5.0 的两个产物都在发布前跑过这一遍，30/30。
...
- **更正 `clean:dist` 的作用范围**：批量删除守卫会在**两个阶段**触发，后果完全不同 ——
  **解包**阶段（删 `win-unpacked` / `win-unpacked.tmp`）是**致命**的，`dist/` 里一个产物都不会有；
  **NSIS 收尾**阶段（删上一版的 `.*__uninstaller.exe`）则产物已生成，只剩一行红字。
